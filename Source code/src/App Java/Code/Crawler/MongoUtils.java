...
public class MongoUtils {
	...
	public void RemoveRecord(BasicDBObject query ){
		this.Collection.remove(query);
	}
	public List<DBObject> Query(BasicDBObject query,BasicDBObject query1){
		List<DBObject> docnument = new ArrayList<DBObject>();
		DBCursor cursor = this.Collection.find(query,query1);
		while (cursor.hasNext()) {
			docnument.add(cursor.next());
		}
		return docnument;
