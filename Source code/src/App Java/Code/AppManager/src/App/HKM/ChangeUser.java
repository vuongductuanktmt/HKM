			...
			public void keyPressed(KeyEvent e) {
				if (e.getKeyCode() == KeyEvent.VK_ENTER) {
					...
					if (this.password_ht.isEmpty() || String.valueOf(this.password_new).isEmpty()
						...
						try {
							ClientToServer CtS = new ClientToServer("check_exist_record","admin","User", "__User__",Login.cache_Login,"",0);
							String request = CtS.getValueRequests();
							if(request.equals("true")){
								 CtS = new ClientToServer("check_exist_record","admin","User", "__Password__",extend.Token(String.valueOf(this.password_ht)),"",0);
								 ...
								 if(request.equals("true")){
									 if (this.password_new.length < 8 || this.password_renew.length < 8) {
											...
											if (!Arrays.equals(this.password_new, this.password_renew)) {
												...
												CtS = new ClientToServer("update","admin","User", "__User__",Login.cache_Login,"__Password__",extend.Token(String.valueOf(this.password_new)),0);
												 request = CtS.getValueRequests();
				...
				if (this.password_ht.isEmpty() || String.valueOf(this.password_new).isEmpty()
					...
					try {
						...
						if (data.CheckExistsRecord(
								new Document("__User__", Login.cache_Login).append("__Password__", extend.Token(password_ht)))) {
							if (this.password_new.length < 8 || this.password_renew.length < 8) {
								errors = "Độ dài mật khẩu phải lớn hơn hoặc bằng 8 kí tự.";
								txtpassword_new.setBorder(new TextBubbleBorder(Color.RED, 2, 4, 0));
								txtpassword_renew.setBorder(new TextBubbleBorder(Color.RED, 2, 4, 0));
								...
								if (!Arrays.equals(this.password_new, this.password_renew)) {
									...
									Document newValue = new Document("__Password__",
											extend.Token(String.valueOf(this.password_new)));
