...
public class TableProductsRecycle extends JFrame implements ActionListener {
	...
	private void UnRemoveCurrentRow() throws AddressException, MessagingException, InterruptedException, URISyntaxException, IOException {
		MsgBox message = new MsgBox(null, "Do you want restore?", true);
		int selectedRow = table.getSelectedRow();
		String title = (String) tableModel.getValueAt(selectedRow, 1);
		if (message.isOk) {
			tableModel.removeRow(selectedRow);
			ClientToServer CtS = new ClientToServer("update","admin","TableWebInfo","__Title__",title,"__Delete__","false",0);
			 CtS.getValueRequests();
		}
	}

	private void UnRemoveAllRows() throws AddressException, MessagingException, InterruptedException, URISyntaxException, IOException {
		...
		if (message.isOk) {
			...
			ClientToServer CtS = new ClientToServer("update","admin","TableWebInfo","null","null","__Delete__","false",0);
			 CtS.getValueRequests();
