...
import App.HKM.Extend.TableData;
...
public class main {
	 ...
	 */
	public main() throws UnknownHostException, InterruptedException {
		initialize();
	}
	...
	private void initialize() throws UnknownHostException {
		...
		extend.CheckCursor(First_Event_Btn, Previous_Event_Btn, Next_Event_Btn, Last_Event_Btn,
				(int) data.PageMax(new Document("__Status__", "Event"), Login.page_size, txtSearch.getText()),
				ModelView.pageNumer__Event);
		extend.CheckCursor(First_Selling_Btn, Previous_Selling_Btn, Next_Selling_Btn, Last_Selling_Btn,
				(int) data.PageMax(new Document("__Status__", "Selling"), Login.page_size, txtSearch.getText()),
		...
		extend.CheckCursor(First_Promotion_Btn, Previous_Promotion_Btn, Next_Promotion_Btn, Last_Promotion_Btn,
				(int) data.PageMax(new Document("__Status__", "Promotion"), Login.page_size, txtSearch.getText()),
		...
		spinner_selling.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent arg0) {
				...
				extend.CheckCursor(First_Selling_Btn, Previous_Selling_Btn, Next_Selling_Btn, Last_Selling_Btn,
						(int) data.PageMax(new Document("__Status__", "Selling"), Login.page_size,
								txtSearch.getText()),
						ModelView.pageNumer__Selling);
			}
		});
		...
		spinner_promotion.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent e) {
				...
				extend.CheckCursor(First_Promotion_Btn, Previous_Promotion_Btn, Next_Promotion_Btn, Last_Promotion_Btn,
						(int) data.PageMax(new Document("__Status__", "Promotion"), Login.page_size,
								txtSearch.getText()),
						ModelView.pageNumer__Promotion);
			}
		});
		...
		spinner_event
				.setModel(new SpinnerNumberModel(
						new Integer(1), new Integer(1), new Integer((int) data
								.PageMax(new Document("__Status__", "Event"), Login.page_size, txtSearch.getText())),
						new Integer(1)));
		spinner_event.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent e) {
				...
				extend.CheckCursor(First_Event_Btn,
						Previous_Event_Btn, Next_Event_Btn, Last_Event_Btn, (int) data
								.PageMax(new Document("__Status__", "Event"), Login.page_size, txtSearch.getText()),
						ModelView.pageNumer__Event);
			}
		});
		...
		Next_Promotion_Btn.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				if (ModelView.pageNumer__Promotion < data.PageMax(new Document("__Status__", "Promotion"),
						Login.page_size, txtSearch.getText())) {
					ModelView.pageNumer__Promotion++;
					spinner_promotion.setValue(ModelView.pageNumer__Promotion);
					modelview.ReloadJlist(list_promotion, model_promotion, "Promotion", txtSearch.getText());
				}
		...
		Last_Promotion_Btn.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				ModelView.pageNumer__Promotion = (int) data.PageMax(new Document("__Status__", "Promotion"),
						Login.page_size, txtSearch.getText());
				spinner_promotion.setValue(ModelView.pageNumer__Promotion);
				modelview.ReloadJlist(list_promotion, model_promotion, "Promotion", txtSearch.getText());
			}
		...
		Next_Selling_Btn.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				if (ModelView.pageNumer__Selling < data.PageMax(new Document("__Status__", "Selling"),
						Login.page_size, txtSearch.getText())) {
					ModelView.pageNumer__Selling++;
					spinner_selling.setValue(ModelView.pageNumer__Selling);
					modelview.ReloadJlist(list_selling, model_selling, "Selling", txtSearch.getText());
		...
		Last_Selling_Btn.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				ModelView.pageNumer__Selling = (int) data.PageMax(new Document("__Status__", "Selling"),
						Login.page_size, txtSearch.getText());
				spinner_selling.setValue(ModelView.pageNumer__Selling);
				modelview.ReloadJlist(list_selling, model_selling, "Selling", txtSearch.getText());
			}
		...
		Next_Event_Btn.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				if (ModelView.pageNumer__Event < data.PageMax(new Document("__Status__", "Event"), Login.page_size,
						txtSearch.getText())) {
					ModelView.pageNumer__Event++;
					spinner_event.setValue(ModelView.pageNumer__Event);
					modelview.ReloadJlist(list_event, model_event, "Event", txtSearch.getText());
		...
		Last_Event_Btn.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				ModelView.pageNumer__Event = (int) data.PageMax(new Document("__Status__", "Event"), Login.page_size,
						txtSearch.getText());
				spinner_event.setValue(ModelView.pageNumer__Event);
				modelview.ReloadJlist(list_event, model_event, "Event", txtSearch.getText());
			}
		...
		groupLayout.setHorizontalGroup(groupLayout.createParallelGroup(Alignment.LEADING)
				...
				.addGroup(
						groupLayout.createSequentialGroup().addGap(125).addComponent(First_Selling_Btn).addGap(18)
								...
								.addComponent(Last_Promotion_Btn, GroupLayout.PREFERRED_SIZE, 30,
										GroupLayout.PREFERRED_SIZE)
								.addGap(41)
								.addComponent(spinner_promotion, GroupLayout.PREFERRED_SIZE, 31,
										GroupLayout.PREFERRED_SIZE)
								.addPreferredGap(ComponentPlacement.RELATED, 172, Short.MAX_VALUE)
								.addComponent(First_Event_Btn).addGap(18)
		...
		list_promotion.addMouseMotionListener(new MouseMotionAdapter() {
			...
			public void mouseMoved(MouseEvent e) {
				JList<?> l = (JList<?>) e.getSource();
				l.setToolTipText(list_promotion.getModel().getElementAt(list_promotion.locationToIndex(e.getPoint()))
						.get__Title__());
			}
		});
		...
		txtSearch.addKeyListener(new KeyAdapter() {
			...
			public void keyPressed(KeyEvent e) {
				if (e.getKeyCode() == KeyEvent.VK_ENTER) {
					...
					ModelView.pageNumer__Selling = 1;
					ModelView.pageNumer__Promotion = 1;
					extend.CheckCursor(First_Event_Btn, Previous_Event_Btn, Next_Event_Btn, Last_Event_Btn, (int) data
							.PageMax(new Document("__Status__", "Event"), Login.page_size, txtSearch.getText()),
							ModelView.pageNumer__Event);
					extend.CheckCursor(First_Selling_Btn, Previous_Selling_Btn, Next_Selling_Btn, Last_Selling_Btn,
							(int) data.PageMax(new Document("__Status__", "Selling"), Login.page_size,
					...
					extend.CheckCursor(First_Promotion_Btn, Previous_Promotion_Btn, Next_Promotion_Btn,
							Last_Promotion_Btn, (int) data.PageMax(new Document("__Status__", "Promotion"),
									Login.page_size, txtSearch.getText()),
							ModelView.pageNumer__Promotion);
					if (data.PageMax(new Document("__Status__", "Selling"), Login.page_size,
							txtSearch.getText()) != 0) {
						spinner_selling.setModel(new SpinnerNumberModel(new Integer(1), new Integer(1),
								new Integer((int) data.PageMax(new Document("__Status__", "Selling"),
										Login.page_size, txtSearch.getText())),
								new Integer(1)));
					} else {
					...
					if (data.PageMax(new Document("__Status__", "Promotion"), Login.page_size,
							txtSearch.getText()) != 0) {
						spinner_promotion.setModel(new SpinnerNumberModel(new Integer(1), new Integer(1),
								new Integer((int) data.PageMax(new Document("__Status__", "Promotion"),
										Login.page_size, txtSearch.getText())),
								new Integer(1)));
					} else {
					...
					System.out.println(data.PageMax(new Document("__Status__", "Promotion"), Login.page_size,
							txtSearch.getText()));
					if (data.PageMax(new Document("__Status__", "Event"), Login.page_size,
							txtSearch.getText()) != 0) {
						spinner_event.setModel(new SpinnerNumberModel(new Integer(1), new Integer(1),
								new Integer((int) data.PageMax(new Document("__Status__", "Event"), Login.page_size,
										txtSearch.getText())),
								new Integer(0)));
					} else {
		...
		groupLayout_1
				.setHorizontalGroup(groupLayout_1.createParallelGroup(Alignment.TRAILING)
						.addGroup(groupLayout_1.createSequentialGroup()
								.addComponent(Icon_Category_Combobox, GroupLayout.PREFERRED_SIZE, 33,
										GroupLayout.PREFERRED_SIZE)
								.addPreferredGap(ComponentPlacement.RELATED)
								.addComponent(Category_Combobox, GroupLayout.PREFERRED_SIZE, 133,
										GroupLayout.PREFERRED_SIZE)
								.addGap(143).addComponent(Icon_Money_Combobox)
								.addPreferredGap(ComponentPlacement.RELATED)
								.addComponent(Money_Comobox, GroupLayout.PREFERRED_SIZE, 133,
										GroupLayout.PREFERRED_SIZE)
								.addGap(160)
								.addComponent(Icon_Hobby_Combobox, GroupLayout.DEFAULT_SIZE, GroupLayout.DEFAULT_SIZE,
										Short.MAX_VALUE)
								.addPreferredGap(ComponentPlacement.RELATED)
								.addComponent(Hobby_Combobox, GroupLayout.PREFERRED_SIZE, 133,
										GroupLayout.PREFERRED_SIZE)
								.addGap(244)
								.addComponent(txtSearch, GroupLayout.PREFERRED_SIZE, 167, GroupLayout.PREFERRED_SIZE)
								.addGap(60)));
